...
abstract class InlineCodegen<out T: BaseExpressionCodegen>(
    ...
    protected fun putArgumentOrCapturedToLocalVal(
        ...
        if (!isDefaultParameter && shouldPutGeneralValue(jvmKotlinType.type, stackValue)) {
            stackValue.put(jvmKotlinType.type, jvmKotlinType.kotlinType, codegen.v)
        }

        if (!asFunctionInline && Type.VOID_TYPE !== jvmKotlinType.type) {
            //TODO remap only inlinable closure => otherwise we could get a lot of problem
            val couldBeRemapped = !shouldPutGeneralValue(jvmKotlinType.type, stackValue) && kind !== ValueKind.DEFAULT_PARAMETER
            ...
            if (capturedParamIndex >= 0) {
                ...
                info = invocationParamBuilder.addNextValueParameter(jvmKotlinType.type, false, remappedValue, parameterIndex)
            }
