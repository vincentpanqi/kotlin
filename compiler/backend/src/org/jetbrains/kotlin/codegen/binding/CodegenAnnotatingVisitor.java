...
class CodegenAnnotatingVisitor extends KtVisitorVoid {
    ...
    public void visitLambdaExpression(@NotNull KtLambdaExpression lambdaExpression) {
        ...
        if (CoroutineUtilKt.isSuspendLambda(functionDescriptor)) {
            ...
            // HACK: we generate two closures: one for inlining only, the other one is for runtime usage.

            // The reason is to get around inliner's inability to inline stateful closures,
            // which itself are passed as parameters to callees.
            // For inlining we generate non-suspend-like closures, and for runtime we generate full-pledged continuation-extending classes.
            // Since inlining closures live only in compile-time, this closure duplication does not affect generated bytecode for
            // non-inlined suspend lambdas: there shall be no inline closures generated in bytecode.
