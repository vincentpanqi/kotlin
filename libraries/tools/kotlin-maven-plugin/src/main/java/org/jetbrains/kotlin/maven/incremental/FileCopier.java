...
public class FileCopier {
    ...
    private void syncDirsImpl(
        ...
        Map<String, FileSnapshot> previousSnapshots = null;
        if (snapshotsStorageFile.isFile()) {
            previousSnapshots = readFileSnapshots(snapshotsStorageFile);
        }
        if (previousSnapshots == null) {
            previousSnapshots = MapsKt.emptyMap();
        }
    ...
    private Map<String, FileSnapshot> readFileSnapshots(@NotNull File inputFile) {
        try (ObjectInputStream input  = new ObjectInputStream(new BufferedInputStream(new FileInputStream(inputFile)))) {
            ...
            Map<String, FileSnapshot> snapshots = new HashMap<>(size);
